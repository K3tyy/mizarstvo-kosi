...
const ServicesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-wood-50">
      ...
      {selectedService ? (
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
               <div>
                  <h2 className="text-3xl font-serif font-bold text-wood-900 mb-6">Zakaj izbrati {selectedService.title.toLowerCase()}?</h2>
                  <p className="text-wood-700 text-lg leading-relaxed mb-8">
                     {selectedService.fullDescription || selectedService.description}
                  </p>
                  
                  {selectedService.benefits && (
                     <div className="bg-white p-8 border border-wood-100 shadow-sm rounded-sm">
                        ...
                        <ul className="space-y-4">
                           {selectedService.benefits.map((benefit, index) => (
                              <li key={index} className="flex items-start">
                                 ...
                                 <span className="text-wood-700">{benefit}</span>
                              </li>
                           ))}
                        </ul>
                     </div>
                  ...
                  <div className="mt-8">
                     <Button onClick={() => document.getElementById('contact-cta')?.scrollIntoView({behavior: 'smooth'})}>
      ...
      {/* CTA Section for Details View */}
      {selectedService && (
         <div id="contact-cta" className="bg-wood-900 py-16 text-center">
            <h2 className="text-3xl font-serif text-white mb-6">Želite takšno {selectedService.title.toLowerCase().replace(' po meri', '')}?</h2>
            <Button variant="white" onClick={() => window.location.hash = '#kontakt'}>
               Kontaktirajte nas
            </Button>
         </div>
      )}
